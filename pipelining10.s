...
.text
		...
		daddi $t1, $zero, 50	#counter of 2nd part out of 10
		daddi $t2, $zero, 100	#counter of 3rd part out of 10
		daddi $t3, $zero, 150	#counter of 4th part out of 10
		daddi $t4, $zero, 200	#counter of 5th part out of 10
		daddi $t5, $zero, 250	#counter of 6th part out of 10
		daddi $t6, $zero, 300	#counter of 7th part out of 10
		daddi $t7, $zero, 350	#counter of 8th part out of 10
		daddi $t8, $zero, 400	#counter of 9th part out of 10
		daddi $t9, $zero, 450	#counter of 10th part out of 10

		lwu $a1, CONTROL($zero)	# preparing #
		lwu $a2, DATA($zero)	# for print #

loop:	lbu $s1, Numbers($t0)	#$t0 is counter of 1st tenth
		daddi $t0, $t0, 1		#counter++
		dadd $s7, $s7, $s1		#add element to the sum (1/10)

		lbu $s2, Numbers($t1)
		daddi $t1, $t1, 1		#counter++
		dadd $s7, $s7, $s2		#add element to the sum (2/10)

		lbu $s3, Numbers($t2)
		daddi $t2, $t2, 1		#counter++
		dadd $s7, $s7, $s3		#add element to the sum (3/10)

		lbu $s4, Numbers($t3)
		daddi $t3, $t3, 1		#counter++
		dadd $s7, $s7, $s4		#add element to the sum (4/10)

		lbu $s5, Numbers($t4)
		daddi $t4, $t4, 1		#counter++
		dadd $s7, $s7, $s5		#add element to the sum (5/10)

		lbu $s1, Numbers($t5)	
		daddi $t5, $t5, 1		#counter++
		dadd $s7, $s7, $s1		#add element to the sum (6/10)

		lbu $s2, Numbers($t6)
		daddi $t6, $t6, 1		#counter++
		dadd $s7, $s7, $s2		#add element to the sum (7/10)

		lbu $s3, Numbers($t7)
		daddi $t7, $t7, 1		#counter++
		dadd $s7, $s7, $s3		#add element to the sum (8/10)

		lbu $s4, Numbers($t8)
		daddi $t8, $t8, 1		#counter++
		dadd $s7, $s7, $s4		#add element to the sum (9/10)

		lbu $s5, Numbers($t9)
		daddi $t9, $t9, 1		#counter++
